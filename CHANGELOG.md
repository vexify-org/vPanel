...
- **压缩：限制堆分配器 + 瘦线程栈 + 惰性回收线程**
  - 启动即设 `MALLOC_ARENA_MAX=1`（glibc 多 arena 合并），堆分配器保留的常驻页显著回落（VmData 9.7MB → 4.2MB）
  - 三个后台线程（监控/插件调度/HTTP worker）改用 `thread::Builder` 缩小栈（256/256/384KB）
  - IotaPanel 空闲回收线程改为**惰性启动**：无插件时不空转一线程，插件首次装上时按需拉起（线程 **5 → 4**）
  - 实测：**常驻 RSS ≈ 2.0MB**（冷启动 ~2.0MB，波动 2.0–2.1MB），四舍五入压到 2MB 以内
- **功能：补齐 ops2 派发**（上一批已注册清单但 `tools/call` 未接线，现全部可用）
- MCP 工具总计仍为 **463**（含 ops/ops2/ops3 三批纯函数工具）
- `mcp` `initialize` 的版本号改用 `CARGO_PKG_VERSION` 自动同步（不再硬编码）
