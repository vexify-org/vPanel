...
//!
//! 内存策略：TLS 相关（rustls/ring/rcgen）是可选的 `tls` 特性。默认精简构建不链
//! 它们（省下 ring 的汇编代码段常驻），`Server` 为纯 TCP 透传的空壳；启用
//! `--features tls` 编译时才是完整的 rustls 服务器。
...
/// 内置 HTTPS 服务器。未启用 `tls` 特性时为空壳：`enabled()` 恒为 false，
/// `accept` 直接透传 TCP 连接，不引入任何 rustls/ring 的常驻代码。
#[cfg(not(feature = "tls"))]
pub struct Server {
    #[allow(dead_code)]
    _x: (),
...
#[cfg(not(feature = "tls"))]
impl Server {
    /// 无需证书配置，恒为未启用 HTTPS 的纯 HTTP 服务器。
    pub fn build(_cfg: &crate::config::Tls) -> io::Result<Server> {
        Ok(Server { _x: () })
    }
...
/// 把 incoming 连接包装为统一流：无 TLS 特性时直接透传 TCP。
#[cfg(not(feature = "tls"))]
pub fn accept(sock: TcpStream, _server: &Server) -> io::Result<Box<dyn Io + Send>> {
    Ok(Box::new(sock))
}

// ======================== 启用 `tls` 特性：完整 rustls 服务器 ========================
...
mod tls_impl {
    ...
    /// 重新导出与精简构建一致的公开 API：`crate::tls::Server` / `crate::tls::accept`。
    ...
    impl RealServer {
        ...
        pub fn build(cfg: &crate::config::Tls) -> io::Result<RealServer> {
    ...
    pub fn accept(sock: TcpStream, server: &RealServer) -> io::Result<Box<dyn Io + Send>> {
