...
impl Ws {
    ...
    pub fn accept(mut reader: TcpStream, head: &str) -> Option<Ws> {
        let key = head
            .lines()
        ...
        let writer = reader.try_clone().ok()?;
        let resp = format!(
            "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: {}\r\n\r\n",
            accept
        );
        reader.write_all(resp.as_bytes()).ok()?;
...
/// 向给定写句柄发送一帧（服务端帧，不掩码）。
fn send_frame(w: &mut TcpStream, opcode: u8, payload: &[u8]) -> std::io::Result<()> {
...
/// 发送二进码数据帧（用于 PTY 输出）。
pub fn send_binary(w: &mut TcpStream, data: &[u8]) -> std::io::Result<()> {
    send_frame(w, 0x2, data)
}

/// 发送 PONG。
pub fn send_pong(w: &mut TcpStream) -> std::io::Result<()> {
    send_frame(w, 0xa, &[])
}

/// 发送关闭帧。
pub fn send_close(w: &mut TcpStream) -> std::io::Result<()> {
    send_frame(w, 0x8, &[])
}
...
/// 精确读取 n 字节，否则返回 Err。
fn read_exact(r: &mut TcpStream, mut buf: &mut [u8]) -> std::io::Result<()> {
    loop {
        match r.read(buf) {
