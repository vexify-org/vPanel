...
pub fn nginx_add(name: &str, server_name: &str, listen: &str, target: &str) -> (bool, String) {
    ...
    // 仅允许合法主机名字符（字母/数字/连字符/点/下划线），
    // 防止把分号、括号等拼进 nginx 配置造成注入。
    let sn = server_name.trim();
    if !valid_server_name(sn) {
        return (false, "server_name 含非法字符（仅允许字母/数字/-/./_）".into());
    }
    let target = target.trim();
    if !target.starts_with("http://") && !target.starts_with("https://") {
        return (false, "反向代理目标需以 http:// 或 https:// 开头".into());
    }
    // 目标会原样嵌入 proxy_pass，禁掉空白与分号/引号/花括号等配置注入字符。
    if !valid_proxy_target(target) {
        return (false, "反向代理目标含非法字符".into());
    }
...
/// server_name 仅允许主机名字符，且不能为空，防止 nginx 配置注入。
...
/// 反向代理 target 会原样嵌入 proxy_pass，禁掉空白与分号/引号/花括号等注入字符。
fn valid_proxy_target(s: &str) -> bool {
    !s.chars()
        .any(|c| c.is_whitespace() || matches!(c, ';' | '"' | '\'' | '{' | '}' | '\\' | '\r' | '\n'))
}
...
mod tests {
    ...
    #[test]
    fn add_rejects_config_injection_in_name_fields() {
        // server_name 带分号/空白/引号要拒绝，防止 nginx 配置注入。
        assert!(!valid_server_name("evil; attacker"));
        assert!(!valid_server_name("evil attacker"));
        assert!(!valid_server_name("evil\"q"));
        assert!(!valid_server_name(""));
        assert!(valid_server_name("ok.example.com"));
        assert!(valid_server_name("a-b_c"));
        // target 带分号/空白/引号/花括号要拒绝。
        assert!(!valid_proxy_target("http://x; server {}"));
        assert!(!valid_proxy_target("http://x y"));
        assert!(!valid_proxy_target("http://x\""));
        assert!(valid_proxy_target("http://127.0.0.1:9000"));
        assert!(valid_proxy_target("http://example.com/path?q=1"));
    }
}