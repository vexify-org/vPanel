...
mod tests {
    ...
    fn add_rejects_config_injection_in_name_fields() {
        ...
        assert!(!nginx_add("ok", "evil; attacker", "443", "http://127.0.0.1:9").0);
        assert!(!nginx_add("ok", "evil attacker", "443", "http://127.0.0.1:9").0);
        assert!(!nginx_add("ok", "ok.example.com", "443", "http://127.0.0.1:9").0);
        // target 带分号/空白/引号/花括号要拒绝。
        assert!(!nginx_add("ok", "ok.example.com", "443", "http://x; server {}").0);
        assert!(!nginx_add("ok", "ok.example.com", "443", "http://x y").0);
        assert!(nginx_add("ok", "ok.example.com", "443", "http://127.0.0.1:9000").0);
    }
}