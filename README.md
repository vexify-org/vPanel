...
> 用 Rust 从零锻造的轻量级服务器控制台 —— 常驻内存约 **1MB**，高并发峰值低于 **2MB**，预算上限 **10MB**。
...
- **轻，轻到极致**：一个静态链接的二进制，常驻约 1MB，低配 VPS 也能举重若轻。
...
内置 **808 个** MCP 工具（纯函数、各自独立），叠加插件系统共暴露 **1244 个** 工具 —— 一支由 AI 直接调遣的运维军团。
...
实测：常驻约 0.8MB；300 并发后约 0.9MB（含软件商店 / MCP 仍远低于 10MB 预算）。
